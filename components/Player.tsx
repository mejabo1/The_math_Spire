...
export const PlayerComponent: React.FC<PlayerProps> = ({ player, isBlocking = false }) => {
  ...
  return (
    <div className="flex flex-col items-center relative group">
       ...
       {isBlocking && (
          <div className="absolute top-0 w-36 h-36 md:w-48 md:h-48 rounded-full border-4 border-blue-400 bg-blue-500/20 animate-block-flash z-20 pointer-events-none mix-blend-screen"></div>
       )}

       {/* Block Icon floating above */}
       {player.block > 0 && (
          <div className="absolute -top-6 md:-top-12 left-1/2 transform -translate-x-1/2 bg-blue-600 text-white px-2 py-0.5 rounded-full shadow-lg border-2 border-blue-400 flex items-center gap-1 font-bold z-10 animate-bounce-slow text-sm md:text-base">
            <Shield size={16} className="fill-white md:w-4 md:h-4" />
       ...
       {/* Poison Icon */}
       {player.poison > 0 && (
          <div className="absolute -top-6 md:-top-12 right-0 bg-green-900 text-green-300 px-2 py-0.5 rounded-full shadow-lg border-2 border-green-500 flex items-center gap-1 font-bold z-10 animate-pulse text-sm md:text-base">
            <FlaskConical size={16} className="md:w-4 md:h-4" />
            <span>{player.poison}</span>
          </div>
       )}

      {/* Character Image - Custom SVG Student Sprite - Responsive Size */}
      {/* Increased Sizes: w-36 h-36 (144px) base, w-48 h-48 (192px) md */}
      <div className="w-36 h-36 md:w-48 md:h-48 lg:w-64 lg:h-64 mb-3 relative filter drop-shadow-[0_0_15px_rgba(59,130,246,0.3)]">
         <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" className="w-full h-full transform scale-x-[-1]">
          ...
          {/* Floor Shadow */}
          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 w-20 md:w-28 h-3 bg-black/40 rounded-[100%] blur-sm"></div>
      </div>

      {/* HP Bar */}
      <div className="w-full max-w-[110px] md:max-w-[140px] relative -mt-4 z-10">
