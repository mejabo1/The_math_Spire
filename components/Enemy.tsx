...
export const EnemyComponent: React.FC<EnemyProps> = ({ enemy, animationState = 'idle', isTargetable = false, onClick, taunt }) => {
  ...
  return (
    <div 
        className={`flex flex-col items-center relative animate-fade-in group mx-1 md:mx-4 ${isTargetable ? 'z-20' : ''}`}
    ...
    >
      ...
      {taunt && (
          <div className="absolute -top-24 md:-top-32 left-1/2 -translate-x-1/2 w-48 md:w-56 z-50 animate-bounce-slow pointer-events-none">
              <div className="bg-white text-slate-900 p-3 md:p-4 rounded-2xl shadow-2xl border-2 border-slate-800 relative">
      ...
      {/* Intent Bubble */}
      <div className="absolute -top-8 md:-top-12 bg-white text-slate-900 px-2 py-0.5 md:px-3 md:py-1 rounded-full shadow-lg border-2 border-slate-300 flex items-center gap-1 md:gap-2 font-bold animate-bounce-slow z-20 scale-90 md:scale-100">
        {enemy.intent.type === 'attack' && <><Sword size={16} className="text-red-600"/> <span className="text-base md:text-xl">{enemy.intent.value}</span></>}
        {enemy.intent.type === 'poison' && <><FlaskConical size={16} className="text-green-600"/> <span className="text-base md:text-xl">{enemy.intent.value}</span></>}
        {enemy.intent.type === 'drain' && <><Droplet size={16} className="text-purple-600"/> <span className="text-base md:text-xl">{enemy.intent.value}</span></>}
        {enemy.intent.type === 'defend' && <><Shield size={16} className="text-blue-600"/> <span className="text-base md:text-xl">{enemy.intent.value}</span></>}
      </div>

      {/* Enemy Image with animation class - Responsive Sizing */}
      {/* Increased sizes: w-24 h-24 (96px) base, w-32 h-32 (128px) md, lg: 40 */}
      <div className={`w-24 h-24 md:w-32 md:h-32 lg:w-40 lg:h-40 mb-3 md:mb-4 relative ${animationClass}`}>
          ...
          <div className="absolute -bottom-4 left-0 w-full z-20">
            ...
            <div className="flex justify-between text-[10px] md:text-xs font-bold text-white mb-0.5 px-1 shadow-black drop-shadow-md">
                <span>{enemy.currentHp}</span>
                <span>{enemy.maxHp}</span>
            </div>
            <div className="h-2 md:h-2.5 bg-gray-900 rounded-full border border-gray-600 overflow-hidden relative">
                <div 
      ...
      <h2 className="text-sm md:text-base font-serif text-red-200 mt-2 tracking-wide drop-shadow-lg text-center max-w-[120px] md:max-w-none leading-tight">{enemy.name}</h2>
    </div>
  );
};
